...
def train_single(model, train_set, test_set, device, criterion, batch_size=1, epoch=1):
    '''
...
def train_joint(model_old, model_new, model_assist, train_set, test_set, device, config):
    ...
    for e in range(config.joint_epoch):
        ...
        train_single(model_assist, dataset_assist, None, device, temp_criterion, 8, 1)  # must be 1
        ...
        loader_new = DataLoader(dataset_new, batch_size=config.new_batch, shuffle=True)
