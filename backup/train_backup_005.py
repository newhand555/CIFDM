...
def train_joint(model_old, model_new, model_assist, train_set, test_set, device, config):
    ...
    criterion_new = AsymmetricLossOptimized().to(device)
    ...
    for e in range(config.joint_epoch):
        ...
        temp_criterion = AsymmetricLossOptimized().to(device)
        # temp_criterion = WeightCorrelationMSELoss(device, config.weight)
        # train assistant model once.
        train_single(model_assist, dataset_assist, None, device, temp_criterion, 8, 12, config.num_workers)  # must be 1
...
def student_train_teacher(teacher_model, train_set, device, config):
    ...
    for x in train_set.data_x:
        ...
        temp = old_front(x)
        data_inter.append(temp.cpu().squeeze(0).detach().numpy())
        data_inter.append(new_front(x).squeeze(0).cpu().detach().numpy())
        data_y.append(old_end(temp).squeeze(0).cpu().detach().numpy())
    ...
    train_single(teacher_front, train_inter, None, device, temp_criterion, config.st_batch, config.st_epoch, config.num_workers)
    # temp_criterion = torch.nn.MSELoss()
    temp_criterion = AsymmetricLossOptimized().to(device)
    # Train whole model to has ability has same performance of combination task from both old and new models.
    train_single(teacher_model, train_all, None, device, temp_criterion, config.st_batch, config.st_epoch, config.num_workers)
